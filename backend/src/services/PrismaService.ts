...
class PrismaService {
  ...
  async createKey(request: CreateKeyRequest) {
    ...
    const VALID_PROVIDERS = ['brave', 'openai', 'claude'];
    ...
    return this.prisma.apiKey.create({
      data: {
        ...
        providers: request.providers.map(p => p.toLowerCase()),
        expiresAt,
      },
    });
  }
  ...
  async verifyKey(keyString: string, provider: string): Promise<boolean> {
    ...
    return apiKey.providers.includes(provider.toLowerCase());
  }
  ...
  async initializeProvider(name: string, data?: UpdateProviderRequest) {
    ...
    try {
      ...
      // Handle unique constraint error - provider already exists
      if (error.code === 'P2002') {
        // Update existing instead
        return this.updateProvider(name, data);
      }
      throw error;
    }
  }
  ...
  async recordUsage(
    ...
    if (!apiKeyId || !providerId) {
      return null; // Silently skip if IDs are missing
    }
    ...
    try {
      ...
      // Silently ignore foreign key or other constraint errors
      // This can happen in tests or if data is inconsistent
      return null;
    }
  }
